...
# Original code by Dr Jan Pearce
# Purpose: to work with Image PPM files
...
'''
    ...
    def PPM_greyscale(self):
        ...
        self.width = len(newpixellist[0])
        self.height = len(newpixellist)
        pixelrow = 0                                                  # starts at row at position 0
        grey_numbers = 0
        for row_list in newpixellist:
            col = 0                                                   # starts at column at position 0
            for pixel in row_list:                                    # goes through the pixels in the list and updates them appropriately
                grey_numbers = newpixellist[pixelrow][col][0] + newpixellist[pixelrow][col][1] + newpixellist[pixelrow][col][2]
                grey_value = grey_numbers/3
                newpixellist[pixelrow][col][0] = int(grey_value)      # updates the red color in rgb
                newpixellist[pixelrow][col][1] = int(grey_value)      # updates the green color in rgb
                newpixellist[pixelrow][col][2] = int(grey_value)      # updates the blue color in rgb
                col += 1                                              # moves to the next column
            pixelrow += 1                                             # moves to the next row
        print(self.outasciifile + " output file turned greyscale.")   # prints the conversion that would have taken place
        self.PPM_updatefrompixellist(newpixellist)                    # This call will update all member attributes appropriately.
    ...
    def PPM_flip_horizontal(self):
        ...
        pixelrow = 0                                        # starts at row at position 0
        for row_list in newpixellist:
            new_row_list = []                               # creates a new empty list
            for pixel in row_list[::-1]:                    # Reads through list backwards so that the pixels can be displayed in the opposite order
                new_row_list.append(pixel)                  # Adds the pixels in the row to the new list with the pixels backwards
            newpixellist[pixelrow] = new_row_list
            pixelrow += 1                                   # moves to the next row
        print(self.outasciifile + " output file flipped.")
        self.PPM_updatefrompixellist(newpixellist)          # This call will update all member attributes appropriately.
    ...
    def PPM_rotateclockwise(self):
        ...
        pixelrow = 0                                        # starts from the row at position 0
        new_list = []
        for b in range(self.height):
            lines_list = []                                 # An empty list is created, into which individual lines will be added.
            for row_list in newpixellist[:: -1]:            # Steps through the list backwards one pixel at a time
                lines_list.append(row_list[pixelrow])       # Adds the one pixel from each row to a list
            pixelrow += 1                                    # Moves to the next row of pixels
            new_list.append(lines_list)                     # Adds the lines list to new list

        print(self.outasciifile + " output file rotated clockwise")
        self.PPM_updatefrompixellist(new_list)              # This call will update all member attributes appropriately.


    def zoom_PPM(self, zoom_amount):
        '''Zooms or enlarges the image by dimensions given by user'''

        newpixellist = self.pixellist
        self.width = len(newpixellist[0]) * int(zoom_amount)        # This sets the height of the new list using user input
        self.height = len(newpixellist) * int(zoom_amount)          # This sets the width of the new list using user input
        bigger_pixel_row = []
        for row_list in newpixellist:
            larger_pixel_row = []                                   # This creates a new list
            for pixel in row_list:                                  # For each pixel in each row
                for i in range(int(zoom_amount)):
                    larger_pixel_row.append(pixel)                  # Adds the pixels to the new list
            for i in range(int(zoom_amount)):                       # Adds to the newer list the amount the image has been zoomed
                bigger_pixel_row.append(larger_pixel_row)
        print(self.outasciifile + " Zoomed.")                       # Prints the action that has been done on the image
        self.PPM_updatefrompixellist(bigger_pixel_row)              # This call will update all member apttributes appropriately.

    def PPM_negative(self):
        '''turns the PPM image into a negative of that image'''

        newlist = self.pixellist
        self.width = len(newlist[0])
        self.height = len(newlist)
        row = 0                                                     # starts from the row at position 0
        for pixline in newlist:                                     # This goes through each rowline
            col = 0                                                 # starts from the column at position 0
            for pixel in pixline:                                   # This goes through each pixel in each row
                newlist[row][col][0] = 255 - newlist[row][col][0]   # Subtracts the pixel from 255 to get the new pixel of the negative

                newlist[row][col][1] = 255 - newlist[row][col][1]   # Subtracts the pixel from 255 to get the new pixel of the negative

                newlist[row][col][2] = 255 - newlist[row][col][2]   # Subtracts the pixel from 255 to get the new pixel of the negative

                col += 1                                            # moves to the next column
            row += 1                                                # moves to the next row
        print(self.outasciifile + " output file turned to negative.")   # Prints the action that has been done on the image
        self.PPM_updatefrompixellist(newlist)                           # This call will update all member attributes appropriately.
################# end of class ##############
